"""
BioMistral-7B PEFT (QLoRA) driver -- TRUE 5-FOLD CV, one job runs all 5 folds on a
...
import biomistral_peft_train_src as src
...
MODEL_KEY = f"biomistral_{args.config}"
...
logger = logging.getLogger("biomistral_peft")
...
def build_quantized_lora_model():
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        ...
        quantization_config=bnb_config, attn_implementation="sdpa")   # SDPA = faster
...
if __name__ == "__main__":
    ...
    logger.info(f"=== BioMistral-7B PEFT [{args.config}] | {DATASET} | TRUE 5-fold ===")
