"""
Training script for BioClinical ModernBERT model using sequence classification.
...
(hsdslab/biomedical-causal-inference). The only additions are:
  ...
  * Saving of each split's learned BioClinical ModernBERT weights (safetensors + tokenizer via
...
NOTE: BioClinical ModernBERT is NOT in the 2026 repo - it is one of the three models
this dissertation adds. It is built by mirroring the repo's BioBERT pipeline exactly
(same optimizer, schedule, batch size, max_steps, early stopping, isotonic calibration,
20-split CV), changing ONLY the checkpoint and output names. Two small API updates in the
_src module (torch AdamW, eval_strategy) are forced by ModernBERT needing transformers>=4.48;
the training LOGIC is unchanged. ModernBERT uses no token_type_ids, handled by the tokenizer
automatically. The pipeline's .lower() call is retained for consistency with the other models.
"""
import numpy as np
import torch
...
parser = argparse.ArgumentParser(description="BioClinical ModernBERT cross-validation training (cluster-ready).")
...
_log_path = os.path.join(RUNLOG_DIR, f"modernbert_{DATASET}_{RUN_TAG}.log")
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s | %(levelname)s | %(message)s",
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler(_log_path)],
)
logger = logging.getLogger("modernbert_train")
logger.info("=== BioClinical ModernBERT training run started ===")
...
model = AutoModelForSequenceClassification.from_pretrained(
    "thomas-sounack/BioClinical-ModernBERT-base", num_labels=2, ignore_mismatched_sizes=True)

# [CLUSTER] log model size (BioClinical ModernBERT ~150M params)
logger.info(f"model loaded: thomas-sounack/BioClinical-ModernBERT-base "
...
def full_process(split, dataset, model, tokenizer):
    ...
    model = AutoModelForSequenceClassification.from_pretrained("thomas-sounack/BioClinical-ModernBERT-base", num_labels=2, ignore_mismatched_sizes=True)
    ...
    df_train, df_dev, df_test = modernbert_train_src.get_split(split, dataset)
    ...
    trainer = modernbert_train_src.train_transformer(tokenizer, model, tokenized_train, tokenized_dev)

    prob_dev, prob_test = modernbert_train_src.predict(trainer, tokenized_dev, tokenized_test)

    prob_test_cal = modernbert_train_src.calibration(prob_dev, df_dev["label"].tolist(), prob_test)
    ...
    df_res["modernbert_temp"] = prob_test
    df_res["modernbert_temp_cal"] = prob_test_cal
    df_res["label"] = df_test["label"]

    # Ensure relative paths for repository portability
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    out_dir = os.path.join(project_root, "dat", dataset, "proc", "cross_val_modernbert_temp")
...
logger.info("=== BioClinical ModernBERT training run finished ===")
logger.info(f"weights   -> {WEIGHTS_DIR}")
logger.info(f"curves    -> {TRAINLOG_DIR}")
logger.info(f"run log   -> {_log_path}")
logger.info(f"preds     -> {os.path.join(PROJECT_ROOT, 'dat', DATASET, 'proc', 'cross_val_modernbert_temp')}")
