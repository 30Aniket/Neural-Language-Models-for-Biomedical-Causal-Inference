...
(hsdslab/biomedical-causal-inference). The only additions are:
  ...
  * Saving of each split's learned ALBERT weights (safetensors + tokenizer via
...
parser = argparse.ArgumentParser(description="ALBERT cross-validation training (cluster-ready).")
...
_log_path = os.path.join(RUNLOG_DIR, f"albert_{DATASET}_{RUN_TAG}.log")
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s | %(levelname)s | %(message)s",
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler(_log_path)],
)
logger = logging.getLogger("albert_train")
logger.info("=== ALBERT training run started ===")
...
def full_process(split, dataset, model, tokenizer):
    ...
    #      i.e. a folder directly reloadable via AlbertForSequenceClassification.from_pretrained(...).
...
logger.info("=== ALBERT training run finished ===")
logger.info(f"weights   -> {WEIGHTS_DIR}")
logger.info(f"curves    -> {TRAINLOG_DIR}")
logger.info(f"run log   -> {_log_path}")
logger.info(f"preds     -> {os.path.join(PROJECT_ROOT, 'dat', DATASET, 'proc', 'cross_val_albert_temp')}")
