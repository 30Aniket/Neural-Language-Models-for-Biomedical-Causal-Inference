"""
Training script for PubMedBERT model using sequence classification.
...
(hsdslab/biomedical-causal-inference). The only additions are:
  ...
  * Saving of each split's learned PubMedBERT weights (safetensors + tokenizer via
...
NOTE: PubMedBERT is NOT in the 2026 repo - it is one of the three models this
dissertation adds. It is therefore built by mirroring the repo's BioBERT pipeline
exactly (same optimizer, schedule, batch size, max_steps, early stopping, isotonic
calibration, 20-split CV), changing ONLY the checkpoint and the output column names.
That keeps it directly comparable with the repo's own models.

Unlike BioBERT, this checkpoint is UNCASED, so the pipeline's .lower() call on
Temp_sentence matches the model's pretraining and is appropriate here.
"""
import numpy as np
import torch
...
parser = argparse.ArgumentParser(description="PubMedBERT cross-validation training (cluster-ready).")
...
_log_path = os.path.join(RUNLOG_DIR, f"pubmedbert_{DATASET}_{RUN_TAG}.log")
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s | %(levelname)s | %(message)s",
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler(_log_path)],
)
logger = logging.getLogger("pubmedbert_train")
logger.info("=== PubMedBERT training run started ===")
...
model = AutoModelForSequenceClassification.from_pretrained(
    "microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract-fulltext", num_labels=2, ignore_mismatched_sizes=True)

# [CLUSTER] log model size (PubMedBERT ~110M params, same class as BioBERT)
logger.info(f"model loaded: microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract-fulltext "
...
def full_process(split, dataset, model, tokenizer):
    ...
    df_train, df_dev, df_test = pubmedbert_train_src.get_split(split, dataset)
    ...
    trainer = pubmedbert_train_src.train_transformer(tokenizer, model, tokenized_train, tokenized_dev)

    prob_dev, prob_test = pubmedbert_train_src.predict(trainer, tokenized_dev, tokenized_test)

    prob_test_cal = pubmedbert_train_src.calibration(prob_dev, df_dev["label"].tolist(), prob_test)
    ...
    df_res["pubmedbert_temp"] = prob_test
    df_res["pubmedbert_temp_cal"] = prob_test_cal
    df_res["label"] = df_test["label"]

    # Ensure relative paths for repository portability
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    out_dir = os.path.join(project_root, "dat", dataset, "proc", "cross_val_pubmedbert_temp")
...
logger.info("=== PubMedBERT training run finished ===")
logger.info(f"weights   -> {WEIGHTS_DIR}")
logger.info(f"curves    -> {TRAINLOG_DIR}")
logger.info(f"run log   -> {_log_path}")
logger.info(f"preds     -> {os.path.join(PROJECT_ROOT, 'dat', DATASET, 'proc', 'cross_val_pubmedbert_temp')}")
